...
export default function TrendingPage() {
  ...
  // Use the cached useMangas hook to fetch all manga data
  const { favoriteMangas: allMangas, isLoading: mangasLoading } = useMangas(
    "system", // Use system user ID for caching
    mangaIds, // Fetch all manga IDs
    []
  );

  const isLoading = isLoadingIds || mangasLoading;

  // Extract unique genres from all mangas
          ...
          <>
            ...
            <div className="mb-6">
              <p className="text-sm text-muted-foreground">
                Showing {displayedMangas.length} of {filteredMangas.length}{" "}
                manga
              </p>
            </div>
